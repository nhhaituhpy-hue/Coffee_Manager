...
import { Home, Zap, Droplets, Wifi, Users, Landmark, MoreHorizontal, Save, Calendar, ChevronRight, Plus, Trash2, Check, Loader2 } from 'lucide-react';
import { AnimatePresence, motion } from 'motion/react';
import { MOCK_FIXED_EXPENSES } from '../constants';

const ICON_MAP: Record<string, any> = {
  Home, Zap, Droplets, Wifi, Users, Landmark, MoreHorizontal
};
...
export const FixedExpenses: React.FC = () => {
  ...
  const [allExpenses, setAllExpenses] = useState<Record<string, any[]>>(() => {
    ...
    return { [defaultMonth]: MOCK_FIXED_EXPENSES.map(e => ({...e, status: 'UNPAID'})) };
  });

  const expenses = allExpenses[selectedMonth] || (selectedMonth === defaultMonth ? MOCK_FIXED_EXPENSES.map(e => ({...e, status: 'UNPAID'})) : []);
    ...
    <div className="space-y-8">
      ...
      <div className="bg-surface rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-outline-variant bg-surface-container flex justify-between items-center">
            <h3 className="text-xs font-bold text-on-surface-variant uppercase tracking-widest">Danh Sách {selectedMonth}</h3>
          ...
          <div className="divide-y divide-outline-variant">
            <div className="grid grid-cols-12 gap-4 px-8 py-4 bg-surface-container text-[11px] font-bold text-outline uppercase tracking-widest">
              <div className="col-span-8">Loại Chi Phí</div>
              <div className="col-span-4 text-right">Số Tiền (VND)</div>
            </div>
